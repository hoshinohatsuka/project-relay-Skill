...
| **E 教学模式** Learn & teach | 给一个想学习的项目 + 学习意图（面向小白：教学模式/学习计划/入门/教程） | `LEARNING_PATH.md`：分阶段学习地图、前置知识清单、鲜活例子走读、第一性原理/对抗式/墨菲三清单、举一反三练习（只给提示不代答）；要深度报告可切模式A |
...
### 模式E 教学模式（v3.1.0 新增）
给一个想学的项目，说："**用教学模式教我看懂这个项目（给链接或本地仓库路径）**"。面向小白（默认，可指定入门/进阶）：E0 先确认项目地址与学习者水平 → 分阶段学习地图（30 分钟认识 → 2 小时核心链路 → 1 天一个模块 → 1 周复刻子集，每阶段带前置知识清单、生词表与**鲜活例子**——真实代码走读）→ **三思维清单**（第一性原理/对抗式审查/墨菲，每条绑项目证据，引导你自己思考、不代答）→ **举一反三练习**（Use→Modify→Debug→Create→Compare，只给提示与验收，答案由你在自己 IDE 产出）→ 社区学习资源。产出 `LEARNING_PATH.md`；想要深度拆解报告就说"改用模式A"。
...
已验证（可复现命令见上节与 `evals/EVALUATION_PLAN.md`）：包完整性校验、触发合同评测、黑板状态校验的正反例，以及六种工作流的人工审查量表。它们证明技能文件、确定性路由合同和状态产物的结构，不证明宿主会自动触发或任意模型会逐字遵守流程。

未验证（missing evidence）：真实宿主自动触发；真实模型在模式 A、B、C、D、E 和续跑中的端到端遵从性；大规模仓库（>500 模块）深度档表现；登录墙平台的社区召回率；模式D 真实双项目借鉴质量与模式E 面向真实小白的学习效果（均依赖宿主与模型）。
