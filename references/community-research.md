...
教学模式（模式E）中，P6 九平台与预算规范不变，检索目标从"评价"转为"给小白找学习资源"：

- 教程：`<项目> tutorial` / `<项目> 入门 教程` / `<项目> 教学 video`
- 坑与避雷：`<项目> 踩坑` / `<项目> beginner 常见错误`
- 作者生态与延伸学习：`<作者> 教学 视频` / `<项目> roadmap`（判断该设计是否稳定、值不值得学）
- 语言/框架前置：`<语言/框架> 从零开始`（前置知识清单的"去哪补"来源）

推荐给用户的资源须标注来源类型（官方/一手用户/媒体/SEO）与发布时间（>2 年标"可能过时"）；知乎/小红书命中营销软文特征时降权或弃用（沿用第 3 节规范）。
