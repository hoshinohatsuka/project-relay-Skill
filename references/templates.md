...
## 10. P7 复核记录与关键分歧
## 11. 报告自查清单（B10，逐项 ✅/⚠️/❌；不适用项必须写明"不适用：<原因>"，禁止静默跳过）
- □ 每条关键事实带 file:line 或 commit SHA 或 URL+日期
- □ 需求→模块映射有封面分母（模块边界规则 + 计数口径）
- □ 选型理由标注证据级别（L1-L4），无纯 L4 冒充
- □ 社区结论区分热度/口碑/技术事实，stars 未当质量分
- □ 覆盖率有分母，未分析范围如实声明
- □ 性能建议区分静态风险/可验证假设/真实 benchmark
- □ 密钥/私有路径未进入产物
---
...
**自查清单（C5 用，B10 规则：不适用项写明"不适用：<原因>"，禁止静默跳过）**：□红线置顶且逐条具体 □状态表与磁盘证据一致 □已完成带证据等级 □测试基线是重跑数字 □下一步无歧义 □无技能附录在 □未提交清单在 □旧文档已归档、指针已更新。
...
## 报告自查清单（B10，逐项 ✅/⚠️/❌；不适用项必须写明"不适用：<原因>"，禁止静默跳过）
- □ 双项目登记表完整（各自路径+分支+commit SHA+角色，D0 锚定）
- □ 差异表每条双端 file:line（绑定各自 SHA）
- □ 借鉴点每条带来源（SHA+file:line+证据级）与落点
- □ 许可证判定记录完整（声明/结构检测/缺失矛盾/结论），恒标"非法律意见"
- □ 意图判定记录在案（借代码 vs 仅理念，用户拍板）
- □ 社区证据带 URL+日期，missing evidence 单列
- □ 执行顺序 Vertical Slice 化，禁区未绕过
```
...
## 覆盖率与未分析范围
## 报告自查清单（B10，逐项 ✅/⚠️/❌；不适用项必须写明"不适用：<原因>"）
- □ 每条三清单条目绑项目证据 file:line
- □ 每阶段带前置知识清单与鲜活例子（file:line + SHA）
- □ 练习只给提示与验收，无完整答案
- □ 学习者水平（E0 确认）与学习目标记录在案
- □ 社区资源带 URL+日期，missing evidence 单列
- □ 覆盖率有分母，未分析范围如实声明
...
## 20. 报告自查清单通则（B10，全模式共用）

**每个模式产出的报告（A 的 LEARNING_REPORT / B 的交接包与收尾 / C 的交接文档 / D 的 BORROWING_PLAN / E 的 LEARNING_PATH）末尾必须带一节"报告自查清单"**，逐项 ✅/⚠️/❌；清单里任何一项**不适用时必须写明"不适用：<原因>"，禁止静默跳过**——这一条本身也是自查项。清单内容按报告类型用对应模板的内置节（模板 8/12/13/14/17），可追加模式特有项。作用是让"完成"自证，而不是让下一位重读全文去猜这报告有没有偷懒。
